...
export function useLiveSessionManager() {
  ...
  const [isMicrophoneMuted, setIsMicrophoneMuted] = useAtom(
    isMicrophoneMutedAtom
  );
  ...
  const updateConversationDialogs = (
    ...
    setConversationDialogs((prev) => {
      ...
      if (index === -1) {
        ...
        newDialogs[index] = {
          ...newDialogs[index],
          content: newDialogs[index].content + content,
        };
        return newDialogs;
      }
    });
  ...
  const updateTokenUsage = (usage: TokenUsage) => {
    setTokenUsage((prev) => {
      ...
      return {
        ...
        input_token_details: {
          cached_tokens:
            prev.input_token_details.cached_tokens +
            usage.input_token_details.cached_tokens,
          text_tokens:
            prev.input_token_details.text_tokens +
            usage.input_token_details.text_tokens,
          audio_tokens:
            prev.input_token_details.audio_tokens +
            usage.input_token_details.audio_tokens,
          cached_tokens_details: {
            text_tokens:
              prev.input_token_details.cached_tokens_details.text_tokens +
              usage.input_token_details.cached_tokens_details.text_tokens,
            audio_tokens:
              prev.input_token_details.cached_tokens_details.audio_tokens +
              usage.input_token_details.cached_tokens_details.audio_tokens,
          },
        },
        output_token_details: {
          text_tokens:
            prev.output_token_details.text_tokens +
            usage.output_token_details.text_tokens,
          audio_tokens:
            prev.output_token_details.audio_tokens +
